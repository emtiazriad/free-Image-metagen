...
import { buildPrompt } from "@/lib/metagen/prompt";
import { parseMetadataText } from "@/lib/metagen/parse";
...
import { getMissingRequiredFields, hasAllRequiredFields } from "@/lib/metagen/validate";
...
export function MetaGenDashboard() {
  ...
  useEffect(() => {
    document.title = "MetaGen AI — Image Metadata Generator";
  }, []);
...
- Do NOT include any other labels, headings, markdown, or explanation.`;
        ...
        let parsed = parseMetadataText(first.text);
...
- Output ONLY the fields (no extra text)`;
        ...
        // If it STILL fails, make it visible as an error instead of silently "Done" with blanks.
        if (!hasAllRequiredFields(parsed)) {
          const missing = getMissingRequiredFields(parsed)
            .map((m) => (m === "altText" ? "ALT_TEXT" : m === "keywords" ? "KEYWORDS" : "CATEGORIES"))
            .join(", ");
