...
serve(async (req) => {
  ...
  try {
    ...
    if (!isJpeg) {
      // Return original file for non-JPEG
    ...
    if (jpegData[0] !== 0xff || jpegData[1] !== 0xd8) {
      return new Response(JSON.stringify({ error: "Invalid JPEG file" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const keywords = metadata.keywords
      ? metadata.keywords.split(/,\s*/).filter((k: string) => k.trim())
      : [];
    ...
    // Create XMP segment for better cross-app compatibility (Title/Description/AltText/Label)
    const xmpSegment = createXMPSegment({
      title,
      // Per request: description/labels/alt text all map to the title
      description: title,
      label: title,
      altText: title,
      keywords,
    ...
    // Embed into JPEG
    const resultJpeg = embedMetadataInJPEG(jpegData, photoshopBlock, exifSegment, xmpSegment);
    ...
    return new Response(resultJpeg, {
      headers: {
        ...corsHeaders,
        "Content-Type": "image/jpeg",
        "Content-Disposition": `attachment; filename="${filename}"`,
        "X-Metadata-Embedded": "true",
      },
    });
  } catch (error) {
    console.error("Error processing image:", error);
    return new Response(JSON.stringify({ error: "Failed to process image", details: String(error) }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
